...
public class InstructionVideoResource {
     ...
     * @return the ResponseEntity with status 201 (Created) and with body the new instructionVideoDTO, or with status 400 (Bad Request) if the instructionVideo has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/instruction-videos")
    @Timed
    public ResponseEntity<InstructionVideoDTO> createInstructionVideo(@RequestBody InstructionVideoDTO instructionVideoDTO) throws URISyntaxException {
        log.debug("REST request to save InstructionVideo : {}", instructionVideoDTO);
        if (instructionVideoDTO.getId() != null) {
            throw new BadRequestAlertException("A new instructionVideo cannot already have an ID", ENTITY_NAME, "idexists");
        }
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the updated instructionVideoDTO,
     * or with status 400 (Bad Request) if the instructionVideoDTO is not valid,
     * or with status 500 (Internal Server Error) if the instructionVideoDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/instruction-videos")
    @Timed
    public ResponseEntity<InstructionVideoDTO> updateInstructionVideo(@RequestBody InstructionVideoDTO instructionVideoDTO) throws URISyntaxException {
     ...
     * @return the ResponseEntity with status 200 (OK) and the list of instructionVideos in body
     */
     ...
     * @return the ResponseEntity with status 200 (OK) and with body the instructionVideoDTO, or with status 404 (Not Found)
     */
