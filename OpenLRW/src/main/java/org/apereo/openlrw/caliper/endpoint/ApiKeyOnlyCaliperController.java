...
public class ApiKeyOnlyCaliperController {
	...
	public ResponseEntity<?> post(@RequestHeader(value = "Authorization") String authorization,
			@RequestBody Envelope envelope) throws OrgNotFoundException {
		...
		if (envelope != null) {
			...
			if (events != null && !events.isEmpty()) {
				...
				for (Event event : events) {
					try {
						String savedId = this.eventService.save(org.getMetadata().get(Vocabulary.TENANT),
								org.getSourcedId(), event);
						ids.add(savedId);
					} catch (Exception e) {
