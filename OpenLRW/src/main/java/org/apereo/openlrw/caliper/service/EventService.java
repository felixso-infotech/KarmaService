...
public class EventService {
	...
	@Autowired
	public EventService(TenantRepository tenantRepository, MongoEventRepository mongoEventRepository,
			UserIdConverter userIdConverter, ClassIdConverter classIdConverter, MongoOperations mongoOperations) {
	...
	public static final ImmutableList<String> STUDENT_ROLES_LIST = ImmutableList
			.of("http://purl.imsglobal.org/vocab/lis/v2/membership#Learner", "student", "Student");
	...
	public String save(String tenantId, String orgId, Event toBeSaved) {
		...
		if (StringUtils.isBlank(toBeSaved.getId())) {
			...
			toBeSaved = new Event.Builder().withAction(toBeSaved.getAction()).withAgent(toBeSaved.getAgent())
					.withContext(toBeSaved.getContext()).withEdApp(toBeSaved.getEdApp())
					.withEventTime(toBeSaved.getEventTime() != null ? toBeSaved.getEventTime() : Instant.now())
					.withFederatedSession(toBeSaved.getFederatedSession()).withGenerated(toBeSaved.getGenerated())
					.withGroup(toBeSaved.getGroup()).withId(UUID.randomUUID().toString().replace("-", ""))
					.withMembership(toBeSaved.getMembership()).withObject(toBeSaved.getObject())
					.withTarget(toBeSaved.getTarget()).withType(toBeSaved.getType()).withTimeZoneOffset(offset).build();
		}

		Tenant tenant = tenantRepository.findById(tenantId).orElse(null);

		MongoEvent mongoEvent = new MongoEvent.Builder().withClassId(classIdConverter.convert(tenant, toBeSaved))
				.withEvent(toBeSaved).withOrganizationId(orgId).withTenantId(tenantId)
				.withUserId(userIdConverter.convert(tenant, toBeSaved)).build();
	...
	public Event getEventForId(final String tenantId, final String orgId, final String eventId) {
		MongoEvent mongoEvent = mongoEventRepository.findByTenantIdAndOrganizationIdAndEventId(tenantId, orgId,
				eventId);
	...
	public Collection<Event> getEventsForClassAndUser(final String tenantId, final String orgId, final String classId,
			final String userId) {
		Collection<MongoEvent> mongoEvents = mongoEventRepository
				.findByTenantIdAndOrganizationIdAndClassIdAndUserIdIgnoreCase(tenantId, orgId, classId, userId);
		if (mongoEvents != null && !mongoEvents.isEmpty()) {
			return mongoEvents.stream().map(MongoEvent::getEvent).collect(Collectors.toList());
		}
		return null;
	...
	public ClassEventStatistics getEventStatisticsForClass(final String tenantId, final String orgId,
			final String classId, boolean studentsOnly) {

		Collection<MongoEvent> mongoEvents;

		if (studentsOnly)
			mongoEvents = mongoEventRepository.findByTenantIdAndOrganizationIdAndClassIdAndEventMembershipRolesIn(
					tenantId, orgId, classId, STUDENT_ROLES_LIST);
		else
			...
			for (String key : eventsByStudent.keySet()) {
				Map<String, Long> eventCountByDate = eventsByStudent.get(key).stream()
						.collect(Collectors.groupingBy(
								event -> StringUtils.substringBefore(event.getEvent().getEventTime().toString(), "T"),
								Collectors.counting()));
		...
		return new ClassEventStatistics.Builder().withClassSourcedId(classId).withTotalEvents(mongoEvents.size())
				.withTotalStudentEnrollments(studentsCounted.keySet().size())
				.withEventCountGroupedByDate(eventCountByDate)
				.withEventCountGroupedByDateAndStudent(eventCountGroupedByDateAndStudent).build();
	 ...
	 */
	public Collection<Event> getEventsForUser(final String tenantId, final String orgId, final String userId,
			final String from, final String to)
			throws EventNotFoundException, IllegalArgumentException, BadRequestException {
		...
		if (from.isEmpty() && !to.isEmpty()) {
			try {
				...
				throw new BadRequestException(
						"Not able to parse the date, it has to be in the following format: `yyyy-MM-dd hh:mm` ");
			}
