...
import org.apereo.openlrw.xapi.XApiAccount;
import org.apereo.openlrw.xapi.XApiActor;
import org.apereo.openlrw.xapi.XApiContext;
import org.apereo.openlrw.xapi.XApiContextActivities;
import org.apereo.openlrw.xapi.XApiObject;
import org.apereo.openlrw.xapi.XApiObjectDefinition;
import org.apereo.openlrw.xapi.XApiResult;
import org.apereo.openlrw.xapi.XApiScore;
import org.apereo.openlrw.xapi.XApiVerb;
...
public class DefaultXapiToCaliperConversionService implements XapiConversionService {
	...
	public Event fromXapi(Statement statement) {
		System.out.println("**********Test fromXapi***" + statement);// ruhail
		...
		if (xapiActor != null) {
			...
			caliperActor = new Agent.Builder().withContext(Context.CONTEXT.getValue()).withName(actorName)
					.withId(actorId).withType(actorType).withExtensions(actorExtensions).build();
		}
		// ACTOR END
		...
		if (xapiResult != null) {
			...
			caliperResult = new Entity.Builder().withId(UUID.randomUUID().toString())
					.withContext(Context.CONTEXT.getValue()).withExtensions(resultExtensions).withActor(actorId)
					.withType("http://purl.imsglobal.org/caliper/v1/Result").withTotalScore(score).build();
		}
		// END Result
		...
		if (xapiObject != null) {
			...
			caliperObject = new Entity.Builder().withId(objectId).withContext(Context.CONTEXT.getValue())
					.withType(objectType).withName(objectName).withDescription(objectDescription)
					.withExtensions(objectExtensions).build();
		}
		// OBJECT END
					...
					if (groupings != null && !groupings.isEmpty()) {
						...
						if (groupings.size() == 1) {
							...
							if (extensions != null) {
								...
								try {
									paramMap = extensions.get(new URI(
							...
							for (XApiObject xo : groupings) {
								...
								if (xapiObjectDefinition != null) {
									if ("http://lrs.learninglocker.net/define/type/moodle/course"
											.equals(xapiObjectDefinition.getType())) {
										type = "http://purl.imsglobal.org/caliper/v1/lis/CourseSection";
										Map<URI, Object> groupExt = xapiObjectDefinition.getExtensions();
										if (groupExt != null) {
											try {
												Object paramMap = groupExt.get(new URI(
						...
						if (grouping != null) {
							...
							if (xapiObjectDefinition != null) {
								...
								if (StringUtils.isBlank(type)
										&& StringUtils.isNoneBlank(xapiObjectDefinition.getType())) {
							...
							if (parents != null && parents.size() == 1) {
								...
								if (parentXapiObjectDefinition != null) {
									...
									subOrganizationOf = new SubOrganizationOf.Builder().withId(parentId)
											.withContext(Context.CONTEXT.getValue()).withType(parentType)
											.withName(parentName).withDescription(parentDescription).build();
								}
							}

							caliperGroup = new Group.Builder().withId(id).withContext(Context.CONTEXT.getValue())
									.withType(type).withName(name).withDescription(description)
									.withExtensions(contextExtensions).withSubOrganizationOf(subOrganizationOf).build();
							...
							caliperGroup = new Group.Builder().withId(parent.getId())
									.withContext(Context.CONTEXT.getValue()).withType(type).withName(name)
									.withDescription(description).withExtensions(contextExtensions).build();
						}
					}
				}
			}
		...
		return new Event.Builder().withAction(caliperAction).withAgent(caliperActor).withObject(caliperObject)
				.withEventTime(instant).withContext(xapiToCaliperType(statement)).withGroup(caliperGroup)
				.withGenerated(caliperResult).build();

	}

	@Override
	public Statement toXapi(Event event) throws URISyntaxException {
		System.out.println("**********Test toXapi***" + event);// ruhail
	...
	private String xApiVerbToCaliperAction(String xapiVerbId) {
		System.out.println("**********Test xApiVerbCaliperAction***" + xapiVerbId);// ruhail
	...
	private String caliperActionToXapiVerb(String caliperAction) {
		System.out.println("**********Test caliperActionToXapiVerb***" + caliperAction);// ruhail
		String verb = null;
		try {
	...
	private String xapiObjectTypeToCaliperEntityType(String xapiType) {
		System.out.println("**********Test xapiObjectTypeToCaliperEntityType***" + xapiType);// ruhail
	...
	private String caliperEntityTypeToXapiObjectType(String caliperType) {
		System.out.println("**********Test caliperEntityTypeToXapiObjectType***" + caliperType);// ruhail
	...
	private String xapiToCaliperType(Statement statement) {
		System.out.println("**********Test xapiToCaliperType***" + statement);// ruhail
		// default to event
