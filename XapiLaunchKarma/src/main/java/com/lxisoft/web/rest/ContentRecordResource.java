...
public class ContentRecordResource {
	 ...
	 *         contentRecordDTO, or with status 400 (Bad Request) if the
	 *         contentRecord has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/content-records")
	@Timed
	public ResponseEntity<ContentRecordDTO> createContentRecord(@RequestBody ContentRecordDTO contentRecordDTO)
			throws URISyntaxException {
		log.debug("REST request to save ContentRecord : {}", contentRecordDTO);
		if (contentRecordDTO.getId() != null) {
			throw new BadRequestAlertException("A new contentRecord cannot already have an ID", ENTITY_NAME,
					"idexists");
		}
	 ...
	 *         contentRecordDTO, or with status 400 (Bad Request) if the
	 *         contentRecordDTO is not valid, or with status 500 (Internal Server
	 *         Error) if the contentRecordDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/content-records")
	@Timed
	public ResponseEntity<ContentRecordDTO> updateContentRecord(@RequestBody ContentRecordDTO contentRecordDTO)
			throws URISyntaxException {
