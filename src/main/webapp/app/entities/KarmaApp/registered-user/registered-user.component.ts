...
export class RegisteredUserComponent implements OnInit, OnDestroy {
  ...
  loadAll() {
    this.registeredUserService
      ...
      .subscribe(
        (res: HttpResponse<IRegisteredUser[]>) => this.paginateRegisteredUsers(res.body, res.headers),
  ...
  trackId(index: number, item: IRegisteredUser) {
    return item.id;
  }
  ...
  private paginateRegisteredUsers(data: IRegisteredUser[], headers: HttpHeaders) {
